...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::hasTable($tableNames['permissions'])) {
            ...
            $this->addUniqueIndexIfMissing($tableNames['permissions'], 'permissions_name_guard_name_unique', ['name', 'guard_name']);
        }
        ...
        if (! Schema::hasTable($tableNames['roles'])) {
            ...
            Schema::table($tableNames['roles'], static function (Blueprint $table) use ($teams, $columnNames) {
                if (($teams || config('permission.testing')) && ! Schema::hasColumn('roles', $columnNames['team_foreign_key'])) {
            ...
            $this->addUniqueIndexIfMissing($tableNames['roles'], 'roles_name_guard_name_unique', ['name', 'guard_name']);
        }
    ...
    private function addUniqueIndexIfMissing(string $table, string $indexName, array $columns): void
    {
        try {
            ...
            // Index already exists or the existing data is incompatible with adding it.
        }
    }
};
